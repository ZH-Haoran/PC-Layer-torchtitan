...
class Attention(nn.Module):
    ...
    def __init__(self, model_args: ModelArgs, layer_id: int):
        super().__init__()
        ...
        wk = nn.Linear(model_args.dim, self.n_kv_heads * self.head_dim, bias=False)
        wv = nn.Linear(model_args.dim, self.n_kv_heads * self.head_dim, bias=False)
        wo = nn.Linear(
    ...
    def init_weights(self, init_std: float):
        # 获取实际的 nn.Linear 层（可能是 PCLinear 或普通的 Linear）
        ...
        for linear in (self.wq, self.wk, self.wv):
            nn.init.trunc_normal_(get_linear(linear).weight, mean=0.0, std=0.02)
        nn.init.trunc_normal_(get_linear(self.wo).weight, mean=0.0, std=init_std)
    ...
    def forward(
        ...
        # PCLinear 会在 forward 中自动应用 preconditioner，直接调用即可
        xv = self.wv(x)
...
class FeedForward(nn.Module):
    ...
    def __init__(
        ...
        # Note: gamma 参数现在由 PCLinear 管理，不再需要在这里创建
        self.gamma_w1 = self.gamma_w2 = self.gamma_w3 = None
    ...
    def init_weights(self, init_std: float):
        ...
        nn.init.trunc_normal_(get_linear(self.w1).weight, mean=0.0, std=0.02)
        for linear in (self.w2, self.w3):
            nn.init.trunc_normal_(get_linear(linear).weight, mean=0.0, std=init_std)
...
class TransformerBlock(nn.Module):
    ...
    def __init__(self, layer_id: int, model_args: ModelArgs):
        ...
        self.attn_residual = ResidualAdd()
        self.ffn_residual  = ResidualAdd()
    ...
    def forward(
        ...
        """
        attn_output = self.attention(self.attention_norm(x), freqs_cis)
        h = self.attn_residual(x, attn_output)

        ffn_output = self.feed_forward(self.ffn_norm(h))
        out = self.ffn_residual(h, ffn_output)
